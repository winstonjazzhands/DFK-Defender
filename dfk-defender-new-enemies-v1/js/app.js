...
(() => {
  ...
  function gameNow() {
    return performance.now() * (game?.gameSpeed || 1);
  }
  ...
  const ENEMY_TEMPLATES = {
    ...
    yellow_baby_boc: { name: 'Baby Boc', hp: 24, damage: 6, moveInterval: 0.2, attackInterval: 0.8, jewel: 3, typeClass: 'runner' },
    blue_baby_boc: { name: 'Storm Baby Boc', hp: 34, damage: 8, moveInterval: 0.2, attackInterval: 0.75, jewel: 4, typeClass: 'runner' },
  };
  ...
  const BOSSES = [
    {
      ...
      jewel: 140,
      abilityInterval: 6,
      phase: { triggerHpPct: 0.25, label: 'Summon the power of HONK' },
      useAbility(enemy, game) {
        const shoved = bossHonkSlam(enemy, game, 70, 1);
        if (shoved) showBanner('Honkules used Honk Slam');
      },
    },
    {
      id: 'motherclucker',
      ...
      jewel: 260,
      abilityInterval: 4,
      minionType: 'yellow_baby_boc',
      minionShieldMultiplier: 0.7,
      useAbility(enemy, game) {
        const spawned = spawnBossMinions(enemy, game, 'yellow_baby_boc', 11, 90);
        if (spawned > 0) showBanner('Motherclucker called the brood');
      },
    },
    {
      id: 'mooseifer',
      ...
      abilityInterval: 6,
      flatReduction: 15,
      phase: { triggerHpPct: 0.5, label: 'Enrage' },
      useAbility(enemy, game) {
        const charged = bossChargeTowardPortal(enemy, game, 2, 80);
        if (charged) showBanner('Mooseifer charged');
      },
    },
    {
      id: 'cluckstorm',
      ...
      jewel: 380,
      abilityInterval: 7,
      minionType: 'blue_baby_boc',
      useAbility(enemy, game) {
        const spawned = spawnBossMinions(enemy, game, 'blue_baby_boc', 11, 75);
        const pushed = bossGalePush(enemy, game, 2, 1);
        if (spawned > 0 || pushed > 0) showBanner('Cluckstorm stirred the gale');
      },
    },
    {
      id: 'honk_and_bonk',
      ...
      flatReduction: 10,
      useAbility(enemy, game) {
        const charged = bossChargeTowardPortal(enemy, game, 2, 95);
        const shoved = bossHonkSlam(enemy, game, 85, 1);
        if (charged || shoved) showBanner('Honk & Bonk caused absolute nonsense');
      },
    },
  ];

  const BOSS_BRIEFINGS = {
    honkules: {
      image: 'assets/honkules.png',
    ...
    mooseifer: {
      image: 'assets/moosifer.png',
      title: 'Mooseifer',
      subtitle: 'Antlers. Violence. Bad intentions.',
      moves: [
        { name: 'Antler Charge', desc: 'Charges hard toward the portal and crushes anything close when he lands.' },
        { name: 'Enrage', desc: 'At 50% HP, Mooseifer doubles his damage, gains +25% speed, turns invincible for 5 seconds, grows 50% larger, pulses red, and heals for 5% max HP every second.' }
      ]
    },
    motherclucker: {
      image: '',
      title: 'Motherclucker',
      subtitle: 'The brood is already moving.',
      moves: [
        { name: 'Brood Swarm', desc: 'Calls in waves of baby bocs to overwhelm weak defenses.' },
        { name: 'Protect the Nest', desc: 'Takes reduced damage while her brood is still alive.' }
      ]
    },
    cluckstorm: {
      image: '',
      title: 'Cluckstorm',
      subtitle: 'Wind, speed, and too many bocs.',
      moves: [
        { name: 'Storm Brood', desc: 'Spits out fast storm baby bocs that can spiral out of control.' },
        { name: 'Gale Push', desc: 'Shoves nearby heroes toward the portal and wrecks fragile positioning.' }
      ]
    },
    honk_and_bonk: {
      image: '',
      title: 'Honk & Bonk',
      subtitle: 'Absolute nonsense has arrived.',
      moves: [
        { name: 'Charge', desc: 'The moose half crashes forward and smashes the front line.' },
        { name: 'Honk Slam', desc: 'The goose half scatters heroes and makes lane control much harder.' }
      ]
    }
  };
  ...
  const els = {
    ...
    skipBossBtn: document.getElementById('skipBossBtn'),
  ...
  const game = {
    ...
    bossBriefingPlan: null,
    bossBriefingWaveStarted: false,
    skipBossUsed: false,
    bossShakeTimeout: null,
    gameSpeed: 1,
  };
  ...
  function getEffectiveTowerRange(tower) {
    const base = Number(tower?.range || 0) || 0;
    return tower?.type === 'archer' ? Math.min(base, 4) : base;
  }
  ...
  function getBossShoutMarkup(text, tone = 'gold') {
    return `<div class="boss-shout boss-shout-${tone}"><span>${text}</span></div>`;
  }

  function skipBossFight() {
    if (!game.bossBriefingPlan || game.runningWave) return;
    const skippedWave = game.bossBriefingPlan.waveNumber;
    game.skipBossUsed = true;
    game.bossBriefingPlan = null;
    game.bossBriefingWaveStarted = false;
    game.introSet = 'intro';
    ...
    syncStatusOverlayVisibility(false);
    showStatusOverlay();
    if (game.nextWavePlan && game.nextWavePlan.waveNumber == skippedWave) {
      game.waveNumber = skippedWave;
      queueNextWave();
    }
    setInstruction(`Honkules was skipped. Click Start Next Wave to continue the journey.`);
    showBanner('Honkules was skipped. The journey continues.', 2200);
    render();
  }

  function getBossBriefingPage() {
    const bossId = game.bossBriefingPlan?.bossId;
    const briefing = BOSS_BRIEFINGS[bossId];
    if (!briefing) {
      return { title: 'Boss Encounter', body: '<p>A boss is waiting. Close this panel to begin the fight.</p>' };
    }
    const art = briefing.image ? `<div class="boss-brief-art-wrap"><img class="boss-brief-art" src="${briefing.image}" alt="${briefing.title}"></div>` : '';
    const moves = briefing.moves.map(move => `<li><span class="intro-highlight">${move.name}</span><span class="boss-brief-move-desc">${move.desc}</span></li>`).join('');
    const honkSkip = bossId === 'honkules'
      ? `<div class="boss-brief-actions"><button type="button" id="bossSkipInlineBtn" class="intro-nav-btn boss-skip-inline-btn">Skip HONKULES</button></div>`
      : '';
    return {
      ...
      body: `
        <div class="boss-brief-card">
          ${art}
          <div class="boss-brief-copy">
            <p class="boss-brief-subtitle">${briefing.subtitle}</p>
            <ul class="boss-brief-moves">${moves}</ul>
            ${honkSkip}
          </div>
        </div>
      `,
  ...
  function getActiveGuidePages() {
    ...
    if (game.introSet === 'boss') return [getBossBriefingPage()];
    return INTRO_PAGES;
  }
  ...
  function updateSpeedButtons() {
    ...
    els.speedButtons.forEach(btn => {
      const active = Number(btn.dataset.speed) === Number(game.gameSpeed || 1);
      btn.classList.toggle('active', active);
      btn.setAttribute('aria-pressed', active ? 'true' : 'false');
    });
  }

  function rebaseGameTimersForSpeedChange(previousSpeed, nextSpeed) {
    const prev = Math.max(1, Number(previousSpeed) || 1);
    const nxt = Math.max(1, Number(nextSpeed) || 1);
    if (prev === nxt) return;

    const ratio = nxt / prev;
    const currentReal = now();
    const fields = [
      'waveStartAt',
      'reductionUntil',
      'invincibleUntil',
    ...
    const rebaseValue = (value) => {
      ...
      return currentReal + ((value - currentReal) * ratio);
    };

    const rebaseBuffMap = (mapObj) => {
      if (!mapObj) return;
      for (const item of Object.values(mapObj)) {
        if (!item || typeof item !== 'object') continue;
        for (const key of ['until', 'nextTickAt']) {
          if (Number.isFinite(item[key]) && item[key] > 0) {
            item[key] = rebaseValue(item[key]);
          }
        }
      }
    };

    const rebaseUnit = (unit) => {
      if (!unit || typeof unit !== 'object') return;
      for (const key of fields) {
        if (Number.isFinite(unit[key]) && unit[key] > 0) {
          unit[key] = rebaseValue(unit[key]);
        }
      }
      rebaseBuffMap(unit.buffs);
      rebaseBuffMap(unit.debuffs);
    };

    if (Array.isArray(game.enemies)) game.enemies.forEach(rebaseUnit);
    if (Array.isArray(game.towers)) game.towers.forEach(rebaseUnit);
    if (Array.isArray(game.pendingSpawns)) {
      for (const spawn of game.pendingSpawns) {
        if (Number.isFinite(spawn.at) && spawn.at > 0) spawn.at = rebaseValue(spawn.at);
        if (Number.isFinite(spawn.spawnAt) && spawn.spawnAt > 0) spawn.spawnAt = rebaseValue(spawn.spawnAt);
      }
    }
    if (Number.isFinite(game.waveStartAt) && game.waveStartAt > 0) {
      game.waveStartAt = rebaseValue(game.waveStartAt);
    }
  }

  function setGameSpeed(multiplier) {
    const safe = Math.max(1, Math.min(3, Number(multiplier) || 1));
    const previous = Math.max(1, Number(game.gameSpeed) || 1);
    rebaseGameTimersForSpeedChange(previous, safe);
    game.gameSpeed = safe;
    updateSpeedButtons();
    showBanner(`Game speed set to ${safe}x`, 1000);
  }
  ...
  function renderIntroPage() {
    ...
    if (game.introSet === 'heroes') {
      if (els.introKicker) els.introKicker.textContent = 'DFK Defense Hero Guide';
      if (els.introTitle) els.introTitle.textContent = 'Meet the Heroes';
    } else if (game.introSet === 'boss') {
      if (els.introKicker) els.introKicker.textContent = 'Boss Encounter';
      if (els.introTitle) els.introTitle.textContent = page.title || 'Boss Encounter';
    } else {
      if (els.introKicker) els.introKicker.textContent = 'DFK Defense Briefing';
      if (els.introTitle) els.introTitle.textContent = 'DFK Defender – Stand Against the Void';
    }

    els.introBody.innerHTML = `<h3 class="intro-page-heading">${page.title}</h3>${page.body}`;

    if (els.introPageLabel) {
      els.introPageLabel.textContent = game.introSet === 'boss'
        ? 'Close this panel when you are ready.'
        : `Page ${game.introPageIndex + 1} / ${pages.length}`;
    }

    if (els.closeIntroBtn) {
      els.closeIntroBtn.setAttribute('aria-label', game.introSet === 'boss' ? 'Start boss fight' : 'Close intro');
      els.closeIntroBtn.title = game.introSet === 'boss' ? 'Start boss fight' : 'Close';
    }

    if (els.introPrevBtn) {
      if (game.introSet === 'boss') {
        els.introPrevBtn.style.display = 'none';
        els.introPrevBtn.disabled = true;
      } else {
        els.introPrevBtn.style.display = '';
        els.introPrevBtn.disabled = game.introPageIndex <= 0;
      }
    }
    ...
    if (els.introNextBtn) {
      if (game.introSet === 'boss') {
        ...
        els.introNextBtn.disabled = true;
      } else {
        els.introNextBtn.style.display = '';
  ...
  function openIntroModal(pageIndex = game.introPageIndex || 0, setName = game.introSet || 'intro') {
    const pages = setName === 'heroes' ? HERO_PAGES : (setName === 'boss' ? [getBossBriefingPage()] : INTRO_PAGES);
  ...
  function triggerScreenShake(intensity = 10, duration = 500) {
    document.documentElement.style.setProperty('--boss-shake-x', `${Math.max(2, intensity)}px`);
    document.documentElement.style.setProperty('--boss-shake-y', `${Math.max(2, Math.round(intensity * 0.6))}px`);
    document.body.classList.add('boss-screen-shake');
    if (game.bossShakeTimeout) clearTimeout(game.bossShakeTimeout);
    game.bossShakeTimeout = setTimeout(() => {
      document.body.classList.remove('boss-screen-shake');
    }, duration);
  }

  function openBossBriefing(plan) {
    const bossPlanItem = plan && plan.enemies ? plan.enemies.find(item => item.bossId) : null;
    game.bossBriefingPlan = bossPlanItem ? { ...plan, bossId: bossPlanItem.bossId } : plan;
    game.bossBriefingWaveStarted = false;
    triggerScreenShake(10, 650);
    openIntroModal(0, 'boss');
  }

  function startPreparedWave(plan) {
    if (!plan || game.runningWave || game.phase !== SETUP_PHASES.BATTLE || game.startingRelicPending) return;
    game.waveNumber = plan.waveNumber;
    game.runningWave = true;
    game.currentPattern = plan.pattern;
    game.activeMutation = plan.mutation;
    game.pendingSpawns = plan.enemies.map(item => ({ ...item, spawned: false }));
    game.waveStartAt = gameNow();
    game.nextWavePlan = null;
    game.relicChoices = [];
  ...
  function closeIntroModal() {
    const closedBossBriefing = game.introSet === 'boss' && game.bossBriefingPlan && !game.bossBriefingWaveStarted;
    game.introOpen = false;
    document.body.classList.remove('intro-open');
    if (els.introModal) {
      els.introModal.classList.add('hidden');
    ...
    syncStatusOverlayVisibility(false);
    showStatusOverlay();
    if (closedBossBriefing) {
      game.bossBriefingWaveStarted = true;
      game.introSet = 'intro';
      setInstruction(`Boss briefing ready. Click Start Next Wave to begin Wave ${game.bossBriefingPlan.waveNumber}.`);
      showBanner('Boss briefing closed. Click Start Next Wave when ready.', 2200);
      render();
    }
  }
  ...
  function getHirePanelSignature() {
    ...
    return JSON.stringify({
      ...
      placingHeroType: game.placingHeroType || null,
      phase: game.phase,
      nextCost: getNextHireCost(),
      bonusHeroHireCharges: game.bonusHeroHireCharges || 0,
      normalAvailable,
      bonusAvailable,
    });
  }

  function refreshHirePanelIfNeeded(force = false) {
    const signature = getHirePanelSignature();
    if (force || signature !== game.lastHirePanelSignature) {
      game.lastHirePanelSignature = signature;
      renderHirePanel();
    }
  }
  ...
  function getAdjacentMoveTargets(tower) {
    if (!tower) return [];
    if (tower.type !== 'warrior') {
      return game.grid
        .filter(tile => isOpenForTower(tile.x, tile.y))
  ...
  function moveTower(tower, nx, ny) {
    if (tower.type === 'warrior' && !adjacentTiles(tower.x, tower.y).some(p => p.x === nx && p.y === ny)) return false;
  ...
  function forceMoveTowerToTile(tower, nx, ny) {
    if (!isOpenForTower(nx, ny)) return false;
    if (!moveWouldPreservePath(tower, nx, ny)) return false;
    const fromTile = tileAt(tower.x, tower.y);
    const toTile = tileAt(nx, ny);
    ...
    tower.moveReadyAt = Math.max(tower.moveReadyAt || 0, now() + 1200);
    return true;
  }

  function moveTowerTowardPortal(tower, steps = 1) {
    let moved = 0;
    for (let i = 0; i < steps; i += 1) {
      const portalTargets = getPortalTargets();
      const next = adjacentTiles(tower.x, tower.y)
        .filter(pos => isOpenForTower(pos.x, pos.y) && moveWouldPreservePath(tower, pos.x, pos.y))
        .sort((a, b) => heuristic(a, portalTargets) - heuristic(b, portalTargets))[0];
      if (!next) break;
      if (!forceMoveTowerToTile(tower, next.x, next.y)) break;
      moved += 1;
    }
    return moved;
  }

  function moveTowerAwayFromEnemy(tower, enemy, steps = 1) {
    let moved = 0;
    for (let i = 0; i < steps; i += 1) {
      const next = adjacentTiles(tower.x, tower.y)
        .filter(pos => isOpenForTower(pos.x, pos.y) && moveWouldPreservePath(tower, pos.x, pos.y))
  ...
  function bossHonkSlam(enemy, gameState, damage = 70, pushTiles = 1) {
    let affected = 0;
    for (const tower of gameState.towers) {
      if (dist(enemy, tower) > 1.5) continue;
      damageTower(gameState, tower, damage, `${enemy.name} honked ${tower.name} backward`);
      const moved = moveTowerAwayFromEnemy(tower, enemy, pushTiles);
      if (moved > 0) affected += 1;
    }
    return affected;
  }

  function spawnBossMinions(enemy, gameState, type, count, delayStep = 100) {
    const spawnCount = Math.max(0, count);
    for (let i = 0; i < spawnCount; i += 1) {
      const minion = createEnemy(type, enemy.spawnLane || chooseLane());
      minion.x = enemy.x;
      minion.y = enemy.y;
      minion.prevX = enemy.x;
      minion.prevY = enemy.y;
      minion.nextMoveAt = now() + 150 + (i * delayStep);
      minion.moveStartedAt = now();
      minion.moveEndAt = minion.nextMoveAt;
      minion.minionOwnerId = enemy.id;
      minion.spawnLane = enemy.spawnLane || chooseLane();
      minion.portalBiasBonus = 1;
      gameState.enemies.push(minion);
    }
    return spawnCount;
  }

  function bossChargeTowardPortal(enemy, gameState, steps = 2, impactDamage = 80) {
    const portalTargets = getPortalTargets();
    if (!portalTargets.length) return 0;
    const path = pathfind({ x: enemy.x, y: enemy.y }, portalTargets);
    if (!path || path.length < 2) return 0;
    const travel = Math.min(steps, path.length - 1);
    enemy.prevX = enemy.x;
    enemy.prevY = enemy.y;
    enemy.x = path[travel].x;
    enemy.y = path[travel].y;
    enemy.moveStartedAt = now();
    enemy.moveEndAt = now() + 220;
    enemy.nextMoveAt = enemy.moveEndAt;
    let hits = 0;
    for (const tower of gameState.towers) {
      if (dist(enemy, tower) <= 1.5) {
        damageTower(gameState, tower, impactDamage, `${enemy.name} barreled into ${tower.name}`);
        hits += 1;
      }
    }
    return hits || travel;
  ...
  function bossGalePush(enemy, gameState, radius = 2, pushTiles = 1) {
    let pushed = 0;
    for (const tower of gameState.towers) {
      if (dist(enemy, tower) > radius) continue;
      const moved = moveTowerTowardPortal(tower, pushTiles);
      if (moved > 0) {
        pushed += 1;
      } else {
        damageTower(gameState, tower, 24, `${enemy.name} slammed ${tower.name} with gale force`);
      }
    }
    return pushed;
  }

  function bossHasLivingMinions(enemy) {
    return game.enemies.some(e => e.minionOwnerId === enemy.id && e.hp > 0);
  }

  function healEnemy(enemy, amount, message = null) {
    if (!enemy || enemy.hp <= 0) return;
    const healed = Math.max(0, Math.min(enemy.maxHp - enemy.hp, amount));
    if (healed <= 0) return;
    enemy.hp += healed;
    createHitFlash(enemy.x, enemy.y, 'priest', `+${Math.round(healed)}`);
    if (message && chance(0.2)) log(message);
  }

  function triggerBossPhase(enemy, current) {
    if (!enemy?.isBoss || !enemy.bossTemplate || enemy.phaseState) return false;

    if (enemy.type === 'mooseifer' && enemy.hp > 0 && enemy.hp <= enemy.maxHp * 0.5) {
      enemy.phaseState = 'enraged';
      enemy.phaseTriggeredAt = current;
      enemy.invincibleUntil = current + 5000;
      enemy.damage = enemy.baseDamage * 2;
      enemy.moveInterval = enemy.baseMoveInterval / 1.25;
      enemy.visualScaleMultiplier = 1.5;
      enemy.visualAura = 'red';
      enemy.buffs.mooseifer_regen = {
        until: Number.POSITIVE_INFINITY,
        enemyHotHeal: enemy.maxHp * 0.05,
        nextTickAt: current + 1000,
      };
      showBanner('MOOSEIFER ENRAGES', 2600);
      log('Mooseifer enraged: damage doubled, speed increased, regeneration started.');
      return true;
    }
  ...
  function getBossVisualState(enemy, current) {
    const basePercent = 160;
    const aura = enemy.visualAura || '';
    const targetMult = enemy.visualScaleMultiplier || 1;

    let growProgress = 1;
    if (enemy.phaseTriggeredAt) {
      const elapsed = Math.max(0, current - enemy.phaseTriggeredAt);
      growProgress = Math.min(1, elapsed / 1600);
      growProgress = 1 - Math.pow(1 - growProgress, 3);
    }
    const sizeMult = 1 + ((targetMult - 1) * growProgress);

    let pulseStrength = 0.016;
    let pulsePeriod = 420;
    if (aura === 'red') {
      pulseStrength = 0.024;
      pulsePeriod = 320;
    ...
    const phase = ((current % pulsePeriod) / pulsePeriod) * (Math.PI * 2);
    ...
    if (aura === 'red') {
      filter = 'drop-shadow(0 0 14px rgba(255,70,70,0.95)) drop-shadow(0 0 30px rgba(255,40,40,0.65)) saturate(1.2)';
    } else if (aura === 'gold') {
      filter = 'drop-shadow(0 0 14px rgba(255,210,90,0.95)) drop-shadow(0 0 28px rgba(255,190,40,0.7)) saturate(1.18) brightness(1.08)';
    }
  ...
  function buildBossWave(waveNumber) {
    ...
    if (boss.id === 'motherclucker') {
      for (let i = 0; i < 4; i += 1) enemies.push({ type: 'yellow_baby_boc', lane, delayMs: 350 + (i * 110) });
    }
    if (boss.id === 'cluckstorm') {
      for (let i = 0; i < 6; i += 1) enemies.push({ type: 'blue_baby_boc', lane, delayMs: 260 + (i * 85) });
    }
    return enemies;
  }
  ...
  function startWave() {
    ...
    const plan = game.nextWavePlan;
    const bossPlanItem = plan.enemies.find(item => item.bossId);

    if (game.bossBriefingPlan && game.bossBriefingWaveStarted && game.bossBriefingPlan.waveNumber === plan.waveNumber) {
      const preparedPlan = game.bossBriefingPlan;
      game.bossBriefingPlan = null;
      game.bossBriefingWaveStarted = false;
      startPreparedWave(preparedPlan);
      return;
    }

    if (bossPlanItem && !(game.bossBriefingPlan && game.bossBriefingPlan.waveNumber === plan.waveNumber && game.bossBriefingWaveStarted)) {
      openBossBriefing(plan);
      return;
    }

    startPreparedWave(plan);
  }
  ...
  function createBossEnemy(boss, laneName) {
    ...
    return {
      ...
      isBoss: true,
      bossTemplate: boss,
      nextAbilityAt: gameNow() + boss.abilityInterval * 1000,
      slowResistance: 0,
      spawnLane: laneName,
      flatReduction: boss.flatReduction || 0,
      invincibleUntil: 0,
      freezeUntil: 0,
      phaseState: null,
      phaseTriggeredAt: 0,
  ...
  function tryAutoCastPriorityAbility(tower, current) {
    if (game.phase === SETUP_PHASES.GAME_OVER) return false;
    if (tower.type !== 'priest' || !isAbilityUnlocked(tower, 'prayer_of_healing')) return false;
    const healReadyAt = tower.abilityReadyAt.prayer_of_healing || 0;
    if (current < healReadyAt) return false;
    const injuredAllies = game.towers
      .filter(t => dist(t, tower) <= 3 && t.hp < t.maxHp)
      .sort((a, b) => (a.hp / a.maxHp) - (b.hp / b.maxHp));
    if (!injuredAllies.length) return false;

    // Priest auto-heal hard fallback: use the same cast path as manual activation
    // so cooldowns, effects, and future edits stay in one place.
    const casted = castAbility(tower, 'prayer_of_healing', { silent: true, auto: true });
  ...
  function tryAutoCastMobileAbility(tower, current) {
    ...
    for (const ability of tower.abilities) {
      if (ability.passive || ability.manualOnly) continue;
      if (tower.type === 'priest' && ability.key === 'prayer_of_healing') continue;
  ...
  function updateTower(tower, delta, current) {
    ...
    if (tryAutoCastPriorityAbility(tower, current)) return;
    ...
    const target = nearestEnemyInRange(tower, getEffectiveTowerRange(tower));
    if (!target) return;
  ...
  function getEnemyAggroTarget(enemy) {
    ...
    const aggroLeash = 2 - (enemy.portalBiasBonus || 0);
    if (towerDistance > portalBiasDistance + aggroLeash) {
      if (enemy.aggroTargetId === bestId) enemy.aggroTargetId = null;
      return null;
    }
  ...
  function updateEnemy(enemy, current) {
    ...
    if (enemy.isBoss && enemy.bossTemplate && enemy.bossTemplate.id === 'honkules' && !enemy.featherFuryApplied && enemy.hp <= enemy.maxHp * 0.5) {
      enemy.featherFuryApplied = true;
      enemy.attackInterval *= 0.75;
      showBanner('Honkules got mad');
    }

    if (enemy.isBoss) {
      triggerBossPhase(enemy, current);
    }
  ...
  function renderEnemyLayer() {
    ...
    for (const enemy of game.enemies) {
      ...
      const enemySize = enemy.isBoss
        ? Math.max(40, Math.min(pos.width, pos.height) * 0.92)
        : getEnemyVisualSize(enemy);
...
if (enemy.isBoss) {
  ...
  const bossSrc = getBossSpriteSrc(enemy);
  if (bossSrc) {
    const img = document.createElement('img');
    img.className = 'enemy-boss-sprite';
    img.src = bossSrc;
    img.alt = enemy.name || 'Boss';
    img.draggable = false;
  ...
  const hpPct = Math.max(0, Math.min(1, enemy.maxHp > 0 ? enemy.hp / enemy.maxHp : 0));
  const bossBar = document.createElement('div');
  bossBar.className = `enemy-healthbar enemy-boss-healthbar ${enemy.type === 'honkules' ? 'enemy-boss-healthbar-feet' : ''}`;
  const bossFill = document.createElement('div');
  bossFill.className = 'fill';
  bossFill.style.width = `${hpPct * 100}%`;
  bossBar.appendChild(bossFill);
  dot.appendChild(bossBar);
...
els.enemyLayer.appendChild(dot);
  ...
  function damageEnemy(sourceTower, enemy, amount, message) {
    if (enemy.invincibleUntil && gameNow() < enemy.invincibleUntil) {
      createAttackLine(sourceTower, enemy, heroColorKey(sourceTower.type));
      createHitFlash(enemy.x, enemy.y, 'default', 'INV');
      return 0;
    }
    let damage = amount;
    if (enemy.reductionUntil && gameNow() < enemy.reductionUntil) damage *= 0.5;
    if (enemy.flatReduction) damage = Math.max(1, damage - enemy.flatReduction);
    if (enemy.isBoss && enemy.bossTemplate?.minionShieldMultiplier && bossHasLivingMinions(enemy)) {
      damage *= enemy.bossTemplate.minionShieldMultiplier;
    }
    enemy.hp -= damage;
  ...
  function tickEffects(unit, current) {
    if (unit.buffs) {
      for (const [name, buff] of Object.entries(unit.buffs)) {
        ...
        if (buff.enemyHotHeal && buff.nextTickAt && current >= buff.nextTickAt && unit.hp > 0) {
          healEnemy(unit, buff.enemyHotHeal, null);
          buff.nextTickAt = current + 1000;
        }
      }
    }
    if (unit.debuffs) {
      for (const [name, debuff] of Object.entries(unit.debuffs)) {
        ...
        if (name === 'eagle_nest' && unit.hp > 0 && debuff.nextTickAt && current >= debuff.nextTickAt) {
          if (!(unit.invincibleUntil && current < unit.invincibleUntil)) unit.hp -= debuff.damage;
        ...
        if (name === 'bleed' && unit.hp > 0 && debuff.nextTickAt && current >= debuff.nextTickAt) {
          if (!(unit.invincibleUntil && current < unit.invincibleUntil)) unit.hp -= (debuff.damagePercent || 0.03) * unit.maxHp;
          debuff.nextTickAt = current + 1000;
        }
      }
    }
  ...
  els.skipBossBtn?.addEventListener('click', skipBossFight);
  ...
  function gameLoop() {
    ...
    try {
      ...
      // Only refresh the hire panel when affordability/availability actually changes.
      renderGrid();
      renderEnemyLayer();
      refreshSelectedPanelLive();
      refreshHirePanelIfNeeded();
      updateTopbar();
      updateMobileBoardFit();
    } catch (error) {
...
// === Boss Asset Mapping (v3) ===
const BOSS_ASSET_SOURCES = {
  honkules: "assets/honkules.png",
  mooseifer: "assets/moosifer.png"
};

function getBossSpriteSrc(enemy) {
  if (!enemy) return '';
  const bossKey = enemy.type || enemy.bossKey || enemy.id || '';
  return BOSS_ASSET_SOURCES[bossKey] || '';
}

// Apply larger sprite size for bosses
function applyBossSprite(entity, el) {
  if (!entity || !el) return;
  const src = getBossSpriteSrc(entity);
  if (!src) return;
  ...
  let img = el.querySelector(".enemy-boss-sprite");
  if (!img) {
    img = document.createElement("img");
    img.className = "enemy-boss-sprite";
    img.alt = entity.name || "Boss";
    img.draggable = false;
    el.appendChild(img);
  }
  img.src = src;
...
// === Boss pulse animation / render safety (v8) ===
(function ensureBossPulseStyle() {
  if (document.getElementById("boss-pulse-style")) return;
  const style = document.createElement("style");
  style.id = "boss-pulse-style";
  style.textContent = `
    .enemy-dot.enemy-boss{
