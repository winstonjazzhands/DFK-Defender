...
async function listWeeklyRunsForWallet(admin: ReturnType<typeof createAdmin>, walletAddress: string, weekKey: string) {
  ...
  while (true) {
    ...
    const { data, error } = await admin
      .from('runs')
      .select('wave_reached, waves_cleared, stats_json, completed_at')
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (claimCount >= 3) return json({ error: 'This weekly bounty is already closed.' }, 409);
    ...
    console.error('claim-bounty fatal error:', error instanceof Error ? { message: error.message, stack: error.stack } : error);
